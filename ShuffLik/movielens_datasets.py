...
for cat_1 in list(genres_items.keys())[:]:
    ...
    if not os.path.exists('data/movielens/'+ cat_1 + '/nodes/'):
        os.makedirs('data/movielens/'+ cat_1 + '/nodes/')
    
    with open('data/movielens/'+ cat_1 + '/nodes/' + cat_1 + '_red_pre_labeled_nodes.tsv', 'w') as f:
        for i in red.iterrows():
            movie = i[1]['movieId']
            item = i[0]
            group = 'red'

            f.write('{}\t{}\t{}\n'.format(movie, item, group))
            
    with open('data/movielens/'+ cat_1 + '/nodes/' + cat_1 + '_blue_pre_labeled_nodes.tsv', 'w') as f:
        for i in blue.iterrows():
            movie = i[1]['movieId']
            item = i[0]
            group = 'blue'
    ...
    with open('data/movielens/'+ cat_1 + '/nodes/' + 'clickstream_weighted_edges.tsv', 'w') as f:
